...
def build_ensemble_result(best_models, *, weights=None, strategy_name="Strategy5_Ensemble", tau=0.75, use_annual=False):
    """
    ...
    return {
        ...
        "client_pattern_weight": tuple(m.get("client_pattern_weight", "N/A") for m in models),
...
def add_monthly_series_feature(
...
def build_growth_rate_features(
    ...
    *,
    ...
    if not feature_cols and not (use_clients and clients_lookup):
        return None
...
def strategy1_ultra_strict_loocv(
    ...
    for n_lags in lags_options:
        ...
        for fb_name, fb_features in feature_blocks.items():
            for lambda_value in pca_lambdas:
                for use_temp in use_monthly_temp_options:
                    for use_clients in use_monthly_clients_options:
                        ...
                        for test_idx in range(n_lags, n_years):
                            fold_training_end = training_end if training_end is not None else years[test_idx] - 1

                            # Training data: all years except test year
                            train_indices = [i for i in range(n_years) if (i != test_idx and years[i] <= fold_training_end)]
                            ...
                            sample_weights = compute_power_weights(
                                train_years, lambda_value
                            )
                            pca_model = fit_weighted_pca(
                                train_scaled, sample_weights, n_pcs
                            )
                            train_pc_scores = weighted_pca_transform(
                                pca_model, train_scaled
                            )
                            ...
                            for pc_idx in range(effective_n_pcs):
                                ...
                                X_train_lags = np.array(X_train_lags).reshape(
                                    len(X_train_lags), -1
                                )
                                ...
                                test_lag_indices = [
                                    i for i in range(test_idx - n_lags, test_idx)
                                ]
                                test_lags_pc = []
                                ...
                                for lag_idx in test_lag_indices:
                                    ...
                                    lag_pc = weighted_pca_transform(
                                        pca_model, lag_scaled
                                    )[0, pc_idx]
                                ...
                                if use_temp and pc_idx > 0:
                                    X_test = add_monthly_temp(
                                        X_test, [years[test_idx]], df_monthly
                                    )
                                if use_clients and pc_idx > 0:
                                    X_test = add_monthly_series_feature(
                                        X_test,
                                        [years[test_idx]],
                                        clients_lookup,
                                    )
                ...
                for test_idx in range(n_lags, n_years):
                    ...
                    for weight in weight_options:
                        if weight is None:
                            ...
                            blended_pattern = (
                                (1 - weight) * mean_curve_normalized
                ...
                for lambda_value in pca_lambdas:
                    for use_clients in use_monthly_clients_options:
                        ...
                        for test_idx in range(n_lags, n_years):
                                ...
                                predicted_pattern = scaler_pca.inverse_transform(
                                    predicted_pattern
                                )[0]
                                ...
                                test_lags_annual = annual_consumption[
                                    test_idx - n_lags : test_idx
                                ]
                                ...
                                if years[test_idx] <= fold_training_end + 1:
                                    ...
                                    X_train_annual_scaled = scaler_annual.fit_transform(
                                        X_train_annual
                                    )
            ...
            for use_clients in use_monthly_clients_options:
                ...
                for test_idx in range(2, n_years):
                    ...
                    if training_window is not None:
                        ...
                        start_idx = 0
                    train_indices = [i for i in range(n_years) if (i < test_idx and i>=start_idx)]
                    train_matrix = monthly_matrix[train_indices]
                    train_annual = annual_consumption[train_indices]
                    ...
                    include_exog = bool(train_features is not None and train_features.shape[1] > 0)
...
def run_analysis_for_entity(
    ...
    **kwargs
    ...
    required = [
        "value_col", "N_PCS", "LAGS_OPTIONS", "FEATURE_BLOCKS",
        "ALPHAS", "PC_WEIGHTS", "R2_THRESHOLD"
    ]
    ...
    monthly_matrix = create_monthly_matrix(df, value_col=value_col)
    years = np.array(sorted(y for y in df['annee'].unique() if y <= eval_years_end))
    ...
    if entity_name == "Activity_Résidentiel":
        tau = under_estimation_penalty / (1 + under_estimation_penalty)
        if all(model is not None for model in (best_s1, best_s2, best_s3)):
            ensemble_candidate = build_ensemble_result([best_s1, best_s2, best_s3], tau=tau, use_annual=False)
            if ensemble_candidate is not None:
                ensemble_results.append(ensemble_candidate)
    else:
        tau = under_estimation_penalty / (1 + under_estimation_penalty)
        if all(model is not None for model in (best_s1, best_s2, best_s3, best_s4)):
            ensemble_candidate = build_ensemble_result([best_s1, best_s2, best_s3, best_s4], tau=tau, use_annual=False)
            if ensemble_candidate is not None:
                ensemble_results.append(ensemble_candidate)
    ...
    if favor_overestimation:
        ...
        all_results = ensemble_results + s1_results + s2_results + s3_results + s4_results
    ...
    monthly_details = {}  # year -> DataFrame (Month, Actual, Predicted, Error_%, [PI_low, PI_high])
    ...
    if eval_years_end in best['valid_years']:
        ...
        for m in range(12):
            ...
            error_pct = np.nan if denom == 0 else ((pred_latest[m] - denom) / denom) * 100
            error_pct_latest.append(error_pct)
            print(f"      {m+1:2d}    | {actual_latest[m]:12,.0f} | {pred_latest[m]:15,.0f} | {error_pct:7.2f}%")
        ...
        annual_error_pct = np.nan if annual_actual == 0 else ((annual_pred - annual_actual) / annual_actual) * 100
        ...
        for i in range(eval_years_start, eval_years_end):
            if i in best['valid_years']:
                ...
                with np.errstate(divide='ignore', invalid='ignore'):
                    err_prev = np.where(actual_prev != 0, (pred_prev - actual_prev) / actual_prev * 100, np.nan)
...
def save_summary(all_results, output_file, monthly_book_file=None):
    ...
    for result in all_results:
        ...
        row = {
            ...
            'PC_Weight': best['pc_weight'] if best.get('pc_weight') is not None else 'N/A',
        ...
        if 'test_years' in result: 
            row.update({k: v for k, v in result['test_years'].items() if k.endswith('annual_ci95')})
            row.update({k: v for k, v in result['test_years'].items() if k.endswith('annual')})
    ...
    if monthly_book_file is not None:
        with pd.ExcelWriter(monthly_book_file, engine="xlsxwriter") as writer:
            ...
            for result in all_results:
                ...
                cols = ["Year", "Month", "Actual", "Predicted", "Error_%", "PI_low", "PI_high"]
            ...
            if index_rows:
                pd.DataFrame(index_rows).to_excel(writer, sheet_name="README", index=False)
